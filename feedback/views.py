...
def feedback_view(request):
    if request.method == 'POST':
        ...
        c.drawString(100, 705, f"{ Question.objects.get(pk=1).question_text}: {question1}")
        c.drawString(100, 690, f"{ Question.objects.get(pk=2).question_text}: {question2}")
        c.drawString(100, 675, f"{ Question.objects.get(pk=3).question_text}: {question3}")
        c.drawString(100, 660, f"{ Question.objects.get(pk=4).question_text}: {question4}")
        c.drawString(100, 645, f"{ Question.objects.get(pk=5).question_text}: {question5}")
        c.drawString(100, 630, f"Additional Feedback: {additional_feedback}")
        c.save()
